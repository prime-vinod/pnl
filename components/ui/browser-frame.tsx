...
export function BrowserFrame({ url, children, className }: Props) {
  return (
    <div className={`overflow-hidden rounded-lg border border-line bg-surface shadow-[0_30px_60px_-20px_rgba(0,0,0,0.5)] ${className ?? ""}`}>
      <div className="flex items-center gap-3 border-b border-line bg-surface px-3 py-2">
        ...
        <span className="ml-2 flex-1 truncate text-center font-mono text-[10px] uppercase tracking-widest text-ink-dim">
          {url ?? "preview"}
        </span>
